...
import BadgeModal, { type BadgeModalBadge } from "./BadgeModal";
...
export default function HomeContent({ disciplines }: HomeContentProps) {
  ...
  const [completionBySlug, setCompletionBySlug] = useState<Record<string, BadgeModalBadge>>({});
  const [selectedBadge, setSelectedBadge] = useState<BadgeModalBadge | null>(null);
  ...
  const activeDisciplineCompletedAt = activeDiscipline
    ? completionBySlug[activeDiscipline.slug]?.completedAt
    : undefined;
  ...
  useEffect(() => {
    ...
    void fetch("/api/user/completions")
      ...
      .then((data: { completions?: CompletionBadgeData[] }) => {
        if (cancelled) return;
        const nextMap: Record<string, BadgeModalBadge> = {};
        for (const row of data.completions ?? []) {
          ...
          nextMap[disc.slug] = {
            id: row.id,
            title: disc.title ?? "Percorso completato",
            completedAt: row.completed_at,
            imgUrl: disc.img_url ?? null,
          };
        }
        setCompletionBySlug(nextMap);
      })
      .catch(() => {
        if (!cancelled) setCompletionBySlug({});
      });
    return () => {
      cancelled = true;
    };
      ...
      <main className="max-w-7xl mx-auto px-1 sm:px-6 lg:px-8 py-12">
        ...
        <div id="discipline-grid" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {disciplines.map((discipline) => {
            const completion = completionBySlug[discipline.slug];
